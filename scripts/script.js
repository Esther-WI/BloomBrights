    ...
    fetch("http://localhost:3000/psychologists")
        ...
        .then(data =>{
            ...
            data.forEach(psy => {
                ...
                li.innerText = `${psy.name} - ${psy.specialty} (Contact: ${psy.contact})`;
...
function fetchPsychologists() {
    ...
    function showPsychologistDetails(psyId) {
        ...
        .catch(error => console.error('Error fetching psychologist details:', error));
        }
    ...
    bookingForm.addEventListener('submit', function(event){
        ...
        .catch(error => {
            bookingStatus.innerText = "Failed to book appointment. Please try again.";
    ...
    fetch("http://localhost:3000/appointments")
        .then(response => response.json())
        .then(data => {
            const userAppointments = data.filter(appointment => appointment.name === userName);
            const appointmentsList = document.getElementById('appointments-list');
            appointmentsList.innerHTML = '';
            userAppointments.forEach(appointment => {
                let li = document.createElement('li');
                li.innerText = `Appointment with ${appointment.psychologistId} on ${appointment.appointmentDate}`;
